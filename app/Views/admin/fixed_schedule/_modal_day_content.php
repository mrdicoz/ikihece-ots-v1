...
<form id="add-fixed-lesson-form">
    ...
    <div class="mb-3">
        ...
        <select name="start_time" class="form-select" required>
            ...
            <?php for ($h = config('Ots')->scheduleStartHour ?? 10; $h < (config('Ots')->scheduleEndHour ?? 18); $h++): ?>
                <option value="<?= str_pad((string)$h, 2, '0', STR_PAD_LEFT) ?>:00"><?= str_pad((string)$h, 2, '0', STR_PAD_LEFT) ?>:00</option>
            <?php endfor; ?>
        </select>
    </div>
    <div class="mb-3">
